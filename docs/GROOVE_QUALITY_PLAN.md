...
| A2 | **2 — mechanism found by measurement; the fix is a detector the duck cannot fool** | **P2.3 the master chain's give-back** | Four renders of the same bass+kick pair, one mastering stage at a time (disco, 2026-09-23), mean \| median dip: **pure −4.41 \| −4.37**, **bus compressor only −2.09 \| 0**, **ceiling only −4.41 \| −4.37**, **file −1.98 \| −0.2**. The ceiling is transparent in both statistics — my gain-recovery hypothesis is dead. The bus compressor takes the *median* onset's dip to **zero** while only halving the mean, and every setting that might explain it is refuted: release (0.22/0.50/0.80 s → −2.09/−2.04/−2.03), threshold (−6 dB → −2.2), ratio (1.2:1 → −2.18). Content cannot pay for it either: a temporary **×3** duck depth moved the pure median to **−13.62 dB** and the file's only to **−0.35** — the median is saturated at zero, not shrunk. So the fix is structural: a compressor whose detector reads a **pre-duck** copy of the bus, which a `DynamicsCompressorNode` cannot do (no external sidechain) and a worklet can. | `duckErasedInMaster` **1 → 0**; the same probe shows the ramp's per-bar level moving with the pattern's 6 dB instead of −1.7 % |
