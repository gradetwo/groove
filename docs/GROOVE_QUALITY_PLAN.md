...
| **5** | **5 — started 2026-09-24** | **P2.5 sample-based texture** (vocal chops, found sound) | The licensing question was the blocker and the project owner has set it aside, so it proceeds. Its two halves are a **product decision** — *what* a sample is for, which is a caller's choice: the model gains a texture role that plays one, and nothing decides for it — and **plumbing**, which has begun: `Gs1Host.importSample` / `clearSample` expose the core's sample import (the processor's `sample` / `sampleClear` messages and `gs_sample_import` have been vendored all along; the adapter's `default:` case said it exposed none of them), returning the core's own codes — 0 ok, 1 too short, 4 no arena room, −1 unsupported — rather than swallowing them. Three cases pin it. | next: a `texture` instrument role that routes a lane to the sample, and a rendering path that loads one |
