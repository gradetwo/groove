...
| # | 事实 | 位置 | 用户感受 |
|---|---|---|---|
| U1 | 首屏是**密集编辑器 + 无单一动作指引**：默认 4 组工具栏全开，桌面约 40 个可见控件，手机 390×844 下工具栏占 **47% 视口**（团队自己的量测） | `Toolbar.tsx:1149/1236/1382` | 第一印象是"控制面板"而不是"做个 beat"——密度部分已修（G.10：36 → 16 控件）；密度部分已修（G.10：36 → 16 控件），**首屏单一动作已完成（v2.0.88）**，见 G.26（桌面/平板；手机端故意不显示，那端由用户重做） |
