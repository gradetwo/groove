...
| A2 | **2 — in progress, two hypotheses eliminated by measurement** | **P2.3 the master chain's give-back** | The largest audible win left, because it **multiplies everything P0.2, P1.1 and B5 already added**. `duckErasedInMaster`'s numbers now say **which stage** owns it — the analyser renders the same bass+kick pair four ways (2026-09-23, disco): sidechain alone **−4.41 dB**, **+ ceiling −4.41 dB** (the ceiling does not touch it — the gain-recovery hypothesis is dead), **+ bus compressor −2.09 dB**, both **−1.98 dB** (median **−0.2**). And the obvious fix is dead too: the compressor's release makes no difference (**0.22 s → −2.09, 0.50 s → −2.04, 0.80 s → −2.03**). What is left is the compressor's *presence*, which the next experiment narrows (threshold/ratio, and whether it is the node's own makeup). | `duckErasedInMaster` **1 → 0**; the same probe shows the ramp's per-bar level moving with the pattern's 6 dB instead of −1.7 % |
