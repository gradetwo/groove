...
| A3 | **3 — partly landed, and the measurement is what says so** | **P2.2 per-note timbre variation** | The nudge exists (`src/audio/noteVariation.ts`: seeded ±6 cents of second-oscillator detune and ±8 % of key-tracked cutoff, keyed by pattern/track/step/note, wired into the offline renderer and the live engine) and the native path is pinned by tests, including the `noteVariation: false` control. **But the measurement found the gap**: the analyser's per-stab comparison renders the same lane twice, nudge on and off, and the two are **identical** (`on 24.69% \| control 24.69% \| delta 0%`) — because the default voice for `chords`/`lead` is **GS-1**, whose host API has `noteOnAt(note, velocity, atFrame, pan)` and no per-note timbre parameters at all. So the variation currently reaches only the native poly synth. The same measurement also showed the metric itself needs care: consecutive *detected* onsets measure pitch, and a window inside the filter sweep measures the transient (35 % vs 25 % settled), which is why the claim is not written yet. | native path pinned; GS-1 per-note params (or a documented re-scope) are the remaining work |
