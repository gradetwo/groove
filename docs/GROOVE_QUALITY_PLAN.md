...
| A2 | **2 ✅ landed — the duck survives the chain** | **P2.3 the master chain's give-back** | The mechanism was measured (a `DynamicsCompressorNode`'s detector sees the ducked programme and hands it back: the *median* onset's dip goes −4.37 dB → **0 dB** through it, the ceiling is transparent, and release/threshold/ratio were all refuted), so the stage is now a **two-input worklet compressor** whose gain follows a **pre-duck copy of the bus** (`GlueCompressor.ts` + `public/glueCompressorWorklet.js`, mirrored sample-for-sample by a test). Every lane taps into that bus from before its own duck gain, offline **and** live, so the two graphs stay the same graph. Measured after: disco's file median dip **−0.2 → −2.39 dB**, chicago-house **−5.03 → −5.03** (untouched), at −1.30 dBTP with the ceiling intact. The makeup is **5.5 dB**, deliberately 0.86 dB below parity with the node, because level parity drives the mix into the ceiling hard enough that the *ceiling* takes the duck back (−1.42 dB at 6.36); the trims are re-recorded once at the end of the batch either way. | `duckErasedInMaster` **1 → 0** (the next sharded run ratchets it), `weakDuck` and the ceiling's own claims unchanged |
