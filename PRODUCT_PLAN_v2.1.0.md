...
| U9 | 中文术语**同一概念 4–5 个名字**：工作室 = 律动工作台/工作台/编曲工作台/编曲台/编曲室；星系 = 星系云团/律动星系/3D 星系图谱/曲风星谱/3D 宇宙星图；音长 = 门限时值/门限/音长/长度/时长 | `common.ts:5,87`；`HelpCenterModal.tsx:353,252`；`studio.ts:387,456` 等 | 教程说的名字，界面上找不到——**已修（v2.1.3）**：用词由用户拍板为**工作台 / 律动星系 / 音长**，47 处替换覆盖 i18n、组件与文档数据，并留下一条**会失败**的术语门禁（键上断言三个正名 + 扫描退役名，附允许表说明哪些"像但不同"的词不能一并禁）。见 G.42 |
...
| ~~限幅器起音平滑~~ | **已尝试并回退，随后结案（M14 / G.34）**：三次实现分别导致天花板泄漏到 +1.9 dBTP 或块长不一致，已恢复原实现并在代码注释与 §3.5 登记为待办——**后来查明：有前瞻时瞬时起音正是让天花板成为硬墙的做法**，真正的缺陷是每样本 O(D) 的滑窗最小值，已换成单调队列（输出逐位不变）。**不再是一条待办。**（此行原文有个被打断的替换，留下「原球istics」这种没意义的字，v2.1.4 顺手修掉。） |
...
### 暂停时的状态（重要）——**已闭环（核实于 v2.1.4）**

> 下面四条是当时的中断状态。**它们已经不是现状**：全量测量后来跑完并写入了基线，159 条 trim 也回填了。
> 核实方式与结果：`node scripts/check_loudness_spread.mjs` 与 `node scripts/check_timbre_spread.mjs` 现在**都通过**
> （6 个类别中位数、69/159 因自身峰均比而顶到上限、全库 p90−p10 3.41 dB、**159 条 trim 与报告完全一致**、
> 0/159 削波、峰值最差 −1.242 dBTP），而且这两个门禁**已经接进 `verify`**（v2.1.4，见 G.43），
> 不会再出现"代码改了、基线没重录、却没人发现"的状态。保留原文只为记录当时发生了什么。
...
## A.4 本轮未做（登记，避免误以为已完成）——**核实于 v2.1.4，三条已不成立**

- **限幅器起音平滑**：见 Q 那一行的说明，三次尝试后回退，需要离线迭代再动。
  → **已由分析结案（M14/G.34）**：有前瞻（3 ms）时瞬时起音正是让天花板成为硬墙的做法，代码注释也这么写；
  真正该修的是那个每样本 O(D) 的滑窗最小值（已换成单调队列，输出逐位不变）。**不再是一条待办。**
- **母带链路改动后的音色基线重录**：`timbre.baseline.json` 尚未重录（母带变了，它一定会漂移）。
  → **已完成**：`node scripts/check_timbre_spread.mjs` 通过，且断言 `src/data/genres/**` 与基线录音逐字节一致。
- **按大类分档的响度门禁**：代码已改，但**尚未有一次成功写入的新基线**验证过它。
  → **已完成**：`check_loudness_spread.mjs` 现在报告 **6 个类别中位数**（Electronic −12.88 / Rock-Metal −9.90 /
  Hip Hop −14.41 / Jazz-Blues −14.09 / Pop-R&B −13.16 / Latin-World −13.69）、69/159 顶到自身峰均比上限、
  trim 钳位 0/159、159 条 trim 与报告一致。两个门禁**已接进 `verify`**（G.43）。
- §3.5 的结构性工作与 §4 的上手改造：**这份清单也已经过时**，逐条核实见 B.5。
...
## B.5 遗留（明确未做）——**核实于 v2.1.4**

1. **限幅器起音平滑**：三次实现分别导致天花板泄漏至 +1.9 dBTP 或块长不一致，已回退并在
   `MasterLimiter.ts` 注释、本文件 §3.5 双处登记。正确做法是离线迭代（构造增益包络使其
   在瞬态离开延迟线前到位），不适合在功能轮次里猜。
   → **已结案，不是待办**：有前瞻时瞬时起音是**正确**做法（M14/G.34），漏的是滑窗最小值那一条，已修。
2. **`achievableLufs` 已从门禁移除**：它是透过限幅器测得的，在曲风顶到天花板后该量已成循环
   定义（会误报「未被拟合」）。门禁改查真正成立的属性，理由写在 `check_loudness_spread.mjs`。
3. §3.5 结构性工作与 §4 上手改造：逐条核实（v2.1.4）——
   **已完成**：齐奏/立体声展开、键盘跟踪（`PolySynth.keyTrackedCutoff`）、立体声完整性（M3/G.20）、
   延迟不重建图（M4/G.31）、`toolbarTiers` 接线（G.10，`probe:toolbar` 在 verify 里）、密度设置落地
   （`SettingsModal` 的 `settings_density_*` + `:root[data-density]`）、首屏单一动作（G.26）、
   iOS 静音无声（U6，`play()` 被 await + `isAudioBlocked()`）。
   **仍未做**：波表与鼓组采样层/轮转（D.5，需采样素材与许可）、混响 IR 移出主线程（M5 已降级为
   「若 profile 显示才做」）。手机端相关的验收目标随用户冻结（G.43 记录）。
...
## C.3 仍未做（下一轮优先级）——**本条随用户冻结（v2.1.4）：手机端相关工作暂停**

> 用户在 v2.1.3 之后明确「手机端相关都暂时冻结」。以下 1–4 条都是手机端 item，**不再推进**，留作解冻后的输入；
> 第 5、6 条的通用部分已在 B.5 逐条核实（多数已完成）。第 3 条的实测尺寸后来也更新为 **36×40**（G.37/G.39）。
...
5. **§3.5 结构性音质工作**与 **§4 上手改造**：**这份清单已经过时**，逐条核实见 B.5——齐奏/键盘跟踪、立体声完整性、
   延迟不重建图、toolbarTiers、密度设置、首屏单一动作、iOS 静音无声**都已完成**；仍未做的只有波表与鼓组采样层
   （D.5，需素材与许可）和混响 IR 移出主线程（M5 已降级）。
6. **限幅器起音平滑**：**已结案**——有前瞻时瞬时起音是正确做法（M14/G.34），真正该修的滑窗最小值已换成单调队列。
...
### 仍未完成（不阻塞「手机端 UI」这一项，属产品规划后续）——**核实于 v2.1.4：本节只余三项**

> 下面四条是当时的状态，**大部分已经不成立**：`toolbarTiers` 已接入渲染路径（G.10，`Toolbar.tsx`
> import `isControlVisible`）、`layout.density` 已有消费者（`useDensityPreference` → `:root[data-density]`）、
> 首屏单一动作已交付（G.23/G.25/G.26）、iOS 静音无声已修（U6：`play()` 被 await + `isAudioBlocked()`）、
> 齐奏/键盘跟踪/立体声完整性/延迟不重建图也都已完成；**限幅器那条是已结案而不是待办**（M14/G.34）。
> **真正仍未做的只有波表与鼓组采样层（D.5，需素材与许可）和混响 IR 移出主线程（M5 已降级）**；
> 第 1 条属手机端，**随用户冻结**。逐条依据见 G.43 与 B.5。
...
## F.6 仍未做（登记，避免误以为已完成）——**核实于 v2.1.4：四条里两条已完成**

> 当时的判断是「附录 E.5 前四条一个没动」。**其中两条已经交付**：unison/立体声展开与立体声完整性
> （M3/G.20，逐条核实见 G.43）。**仍未做**：波表 `createPeriodicWave`、鼓组采样层与每击轮转
> （D.5，需引入采样素材与许可）。§4 上手改造也已完成（G.26 等，见 B.5）；底部双层栏随用户
> 冻结手机端（G.43）。保留原文只为记录当时的判断。
...
**一条测试基建的缺口**：`check_timbre_spread.mjs` / `check_loudness_spread.mjs` 读的是**已提交的报告**，
从不重新渲染，所以交付前必须判断基线是否需要重录，否则门禁会脱节（本仓库已有这条规矩，此处只是重申）。
（**v2.1.4 / G.43**：这两个门禁此前**不在 `verify` 里**，已接进去——「门禁存在但没人跑」比「报告会脱节」
更危险。原文这里连脚本名都写错了，实际文件名带 `_spread`。）
...
## G.4 仍未做（更新）——**核实于 v2.1.4：第 2、3 条已不成立**

1. **手机端底部双层栏**：标签栏 53px + 走带条 59px = 664px 视口的 17%。合并方案已想清
   （走带并入标签栏），但需要真机量测，留到下一轮。
   → **v2.1.4：随用户冻结手机端，不再推进**；解冻后作为第一批输入之一。
2. ~~**工具条分层（`toolbarTiers.ts`）仍未接线**~~ **已完成（G.10）**：`Toolbar.tsx` 现在 import
   `isControlVisible`（含 Tier 2/3 的可见性判定），且 `probe:toolbar` 就在 `verify` 里——
   所以「工具条多出一个按钮而没人发现」这件事**已经会让门禁变红**，原文那句在 G.10 之后就不再成立。
3. **波表 `createPeriodicWave`、鼓组采样层与每击轮转、立体声完整性其余部分**（附录 E.5 第 2–4 条）。
   → **v2.1.4：只剩前两项**（D.5，需采样素材与许可）；~~立体声完整性其余部分~~ **已完成**
   （M3/G.20 声像与合唱、M4/G.31 延迟不重建图）。
4. **`check_timbre_spread.mjs` / `check_loudness_spread.mjs` 读的是已提交报告，从不重新渲染**——
   ...
   → **v2.1.4（G.43）**：这两个门禁此前**根本没有 npm script、也不在 `verify` 里**，只能靠人记得手动跑；
   比「报告会脱节」更危险的是「门禁没人跑」。现在它们是 `check:loudness` / `check:timbre`，接在 `verify`
   的 `check:gs1` 之后、E2E 之前（0.06 s / 0.15 s，不启浏览器），基线一旦与代码脱节，发版就会失败。
...
### 仍未做：横屏的彻底解法——**随用户冻结（v2.1.4）：手机端不再推进**

> 下面的分析连同「不能简单地在横屏隐藏标签栏」这个结论，留给手机端重设计时直接用；
> 本文件不再把手机端项当作待办推进（用户已冻结手机端）。
...
- 鼓组采样层与每击轮转（E.5 第 3 条，D.5，需采样素材与许可）。
- ~~立体声完整性其余部分（E.5 第 4 条）~~ —— **已完成**：声像前置分接（G.19）、合唱不再降混单声道
  （G.20）、延迟不重建图（M4/G.31）；混响 IR 移出主线程（M5）已降级为「profile 显示需要才做」。
...
4. **音色侧剩余两条**（附录 E.5 第 3–4 条）：
   ...
   - **立体声完整性其余部分**：~~混响 IR 移出主线程~~（M5，已降级为「profile 显示需要才做」）、
     ~~延迟不重建图~~ —— **已完成（M4/G.31，v2.0.92）**：每次构建用一代 gain 交叉淡化，退休的图在音频时钟上扫掉。
...
8. **PC/iPad 的走带条会随页面滚走**，而手机版已固定 —— **性质待确认**：
   ...
   → **核实于 v2.1.7（G.46）**：那句注释**在仓库里不存在**（搜「遮挡 / 长页面 / scroll away」无命中），
   所以这条的前提本身就是错的。核实过程中先查出一个**更严重的缺陷并已修复**：≥1024px 时工作室的两列
   **左右互换**（`lg:grid-cols-[352px_1fr]`，编辑器被挤进 352px 列、信息栏占 1012px），根因是 v2.0.88
   加首屏提示时把 `order-1 lg:order-2` 留在了 `section` 上（它不是网格项）；修复后 1440×900 为
   信息栏 352 左 / 编辑器 1012 右。**修完再测**：PC/iPad 上走带条仍会随页面滚走（1440×900 滚到底后
   走带组 `top −247`，`elementFromPoint` 为空；量测脚本 `scripts/probe_transport_reach.mjs`）。
   **是否把它钉在页眉之下（工具栏行 105px ≈ 12% 视口）属于产品取舍，留给用户拍板**；
   手机端已冻结，不受影响。
   → **已拍板并实现（v2.1.8 / G.47）**：用户选择「只钉紧凑走带条」。实现时又发现 `position: sticky`
   在本应用里**从未生效**（`<body>` 上的 `overflow-x-hidden` 让它成了滚动容器），一并修掉；
   现在走带条常驻页眉之下（约 52px），页眉本身也第一次真的吸顶。
...
- **已完成（核实于 v2.1.4，这里此前写着"仍未做"，与上面的 U 表自相矛盾）**：**首屏单一动作**（U1/U2）
  由 v2.0.85（G.23，引导第 1 页的「先听一下这个曲风」）与 v2.0.88（G.26，首屏提示「先听一遍/Listen」）交付；
  「点一个格子」那半步由 v2.0.87（G.25，实操教学指向真实控件）交付。**手机端故意不显示首屏提示**
  （那端已由用户冻结、将由用户重做）。
- 同一条里列的三项也**都已交付**：术语表 = U9（**v2.1.3**，G.42）、工具栏 `?` 入口 = v2.0.89（G.28）、
  Modal 栈 = v2.0.86（G.24）。
...
（非模态高亮 + 指向某个 step cell），与现在的模态结构是两件事——**这一半后来也做了**：v2.0.87（G.25）
把实操教学的每一步指向**真实控件**（压暗其余、圈出目标、`pointer-events: none`），用户确实是在编辑器里点。
U1（应用首屏本身没有单一动作）**不再是未做**：v2.0.88（G.26）在首屏给了单一动作「先听一遍 / Listen」
（桌面与平板；手机端故意不显示，那端由用户冻结后重做）。
...
| **U9** 中文术语一名多译 | **已完成（v2.1.3）**：用户拍板 **工作台 / 律动星系 / 音长**，47 处替换（含 6 个断言旧文案的测试），并留下术语门禁。见 G.42 |
| **U10** 卷帘是无障碍空洞 | **已完成（v2.0.93 主体 + v2.1.5 尾巴，见 G.32 / G.44）**：可聚焦表面（`role="application"` + 键盘说明 + `role="status"` live region），键盘光标 + Enter/Delete/`+`/`-`/`[`/`]` 完成建、删、力度、音长，移动与编辑全部播报；音符改为 `aria-hidden`（无行可放 `gridcell`，原来的 `role="grid"` 是无效 ARIA）。方向键在**有选区时仍归选区**（原有 window 级行为一行未改）。**v2.1.5 补完三条尾巴**：参数条成为一个 tab 停留点 + 光标（每列 `role="slider"`，上下键改值、左右键换列、全部播报），长度 `[`/`]` 与力度一样作用于**整个选区**（新增 `scaleNotesLength`），左侧钢琴键可用键盘行走并试听（roving tabindex + Enter/Space 重弹） |
...
## G.41 手机档进了发布门禁：`verify` 现在跑 7 个目标（v2.1.2）

**一条不在发布链里的门禁，就是一条迟早会被跳过的门禁。** 手机端已经是这个产品的一半界面（11 个面的 44 px 规则、
...
变成约 11 min，其中手机档 ~290 s（56 + 65 + 82 + 86）。

### 先修掉那条已知的 flake，再把它放进门禁

把一个"偶发失败"的检查放进发布门禁是不负责任的，所以先按 G.40 记下的线索（`100 → 52`，方向反了）找到真因——
它不是"拖拽偶发"，而是**比较了两个不同的音符**：

1. 改前读的是**元素句柄**（`barEl.getAttribute`），改后却用**选择器重新查询** `[data-testid^='piano-roll-velocity-bar-']`。
   这次运行前面已经编辑过 pattern，力度条的顺序/集合会变，于是两个数字来自不同音符，比较本身没有意义——
   这正是"向上拖却变小"的来源。
2. `scrollIntoViewIfNeeded()` + 睡 150 ms **不是**"布局已稳定"的承诺（平板横屏尤其慢）：现在把盒子反复读到你它
   不再移动为止。
3. 按下之前用 `elementFromPoint` **核实落点就在这根条上**——和 `clickVerified` 对点击用的是同一套"验证送达"契约。

修完连跑两次全档，7 个目标全绿。

### 顺带核对：目标里的两条要求其实早有测试

本轮原计划给"禁止缩放/放大镜"和"接近原生（无 300 ms 延迟）"补测试，动手前先查了一遍——**都已经有了**：
`deviceCapabilities.test.ts` 断言 viewport meta 里的 `maximum-scale=1` + `user-scalable=no`（并记录了为什么是刻意的），
`mobileShell.test.tsx` / `mobileTransportBar.test.tsx` 用 `fireEvent.pointerUp` 断言底栏**在 pointerup 提交**而不是
click。所以本轮**没有**新增重复测试，只把这段核对写在这里：先看有没有，再决定加不加。

## G.42 U9 中文术语：工作台 / 律动星系 / 音长，并用门禁钉住（v2.1.3）

用词由用户拍板：**工作室 → 工作台**、**星系 → 律动星系**、**音符时长 → 音长**。

### 先量，再改

| 概念 | 退役的名字（改动前出现次数：i18n / 源码） |
|---|---|
| 工作室 | 律动工作台 3/0、编曲工作台 8/7、编曲台 4/5、编曲室 1/3 |
| 星系 | 星系云团 1/1、星系图谱 3/4、曲风星谱 1/0、宇宙星图 1/0 |
| 音长 | 门限时值 1/0、门限（= 音符门限）3/7、时长 1/2、长度（= 音符门限）7/2 |

**47 处替换**，覆盖 8 个 i18n 文件、`HelpCenterModal`、`KickAnatomyView`、`VelocityLane`、
`LanguageContext`、`tutorialCourses`、`uiManualItems`，以及 **6 个断言旧中文文案的测试**（它们正是"改了界面却忘了
教程/测试"的来源）。桌面与手机共用这些字符串，**手机端代码一行未动**（用户已冻结手机端）。

### 刻意不动的东西（一张允许表，而不是把规则放宽）

扫描器如果分不清"同名不同义"，就只能一路放宽到什么都不查。所以这些**保留**并写进注释与测试：

- **门限混响**（gated reverb）是效果器术语，159 个曲风的文案里到处都是——只禁"门限"的**音符门限**语义
  （用 `门限(?!混响|军鼓|卡顿)`）。
- **长度**指音轨循环长度/步长（polymeter）时保留；只有**音符长度**才叫音长（`roll_resize_hud`、
  `roll_quantize_lengths`、`chords_arp_gate` 这几个键改成音长）。
- **物理长度**（大师课里的比对）、**衰减长度**（次低频尾音）保留。
- 曲风的 **origin_place 里"工作室"**＝"个人音乐工作室"，是一个**地点**而不是这个应用的编曲页 → 保留；
  扫描范围也因此把 `src/data/genres/**` 排除在外（那里是音乐描述，不是界面文案）。

### 门禁（`terminology.test.ts`，3 条）

1. **键上断言正名**：`nav_studio.zh === "工作台"`、`nav_galaxy.zh === "律动星系"`、`roll_length.zh === "音长"`，
   并在三个"长度"键上断言含"音长"——这样"改成第三个新词"也会失败，而不只是"改回旧词"才失败。
2. **扫描退役名**：在界面文案范围内（i18n、components、views、data、features，排除 genres）扫上面那张表 +
   音符门限语义的"门限"，任何一处命中都失败，并报出 `文件:行号 词`。
3. **扫描器自检**：对 `短促门限混响军鼓`、`循环长度`、`物理长度` 必须**放行**，对 `返回编曲台`、`门限: {gate} 步`
   必须**命中**——没有这条，前两条只是"当前为真的断言"，不是检查。

### 顺带发现的重复

扫描时抓到 `LanguageContext.tsx` 里**另有一份**应用标题字符串（"音乐曲风探索与律动工作台"），i18n 里还有
`app_title` 的同义版本；`help.ts` 里也藏着两处没被最初的替换清单覆盖（"一键烘焙入编曲工作台"、
"一键烘焙至编曲台与三维流派引力传承图谱"）。**这正是门禁存在的理由**：手动清单总会漏，
而扫描器不会。

全量单测 **187 文件 / 2133 用例**（+1 文件 3 用例），`npm run verify` 全绿（含 3 桌面 + 4 手机目标）。

## G.43 两个音质门禁接进 verify；顺带把计划里"仍未做"的过时说法清掉（v2.1.4）

### 起因：一次"计划核对"，不是功能轮次

手机端冻结、D.5 需要素材之后，我按老规矩把计划里所有"未做/仍未做/剩余"的说法逐条拿代码核了一遍。
**结果有六处过时，其中一处还和同一份文档的另一处互相矛盾。** 这种过时不是排版问题：它会让人（包括我）
重复做已完成的工作，或者把"其实早就做完"当成阻塞写进汇报。

### 接进 verify 的两个门禁（本轮唯一改变行为的改动）

`scripts/check_loudness_spread.mjs` 与 `scripts/check_timbre_spread.mjs` 一直存在、一直通过——
但**从来没有 npm script，也从来不在 verify 里**，只能靠人记得手动跑。它们只读已提交的 JSON 与源码、
**不启浏览器**，实测 0.06 s 与 0.15 s。所以：加 `check:loudness` / `check:timbre` 两个 script，并接进 `verify`
（在 `check:gs1` 之后、E2E 之前）。**"代码改了、基线没重录"这件事，从此每次发版都会被抓住。**

### 核实结果（逐条替换掉计划里的过时断言）

| 计划原来的说法 | 核实后 |
|---|---|
| §A.4「全量测量被暂停、未写报告、trim 未回填、`check_loudness_spread` 会失败」 | **已闭环**：两个门禁都通过——6 个类别中位数（Electronic −12.88 / Rock-Metal −9.90 / Hip Hop −14.41 / Jazz-Blues −14.09 / Pop-R&B −13.16 / Latin-World −13.69）、69/159 顶到自身峰均比、trim 钳位 0/159、**159 条 trim 与报告完全一致**、0/159 削波、峰值最差 −1.242 dBTP |
| §A.4 / §B.5 / §C.3「母带改动后音色基线未重录」 | **已完成**：`check_timbre_spread` 通过，并断言 `src/data/genres/**` 与基线录音逐字节一致 |
| §A.4 / §B.5 / §C.3「限幅器起音平滑：三次尝试后回退，需离线迭代」 | **已结案，不是待办**：有前瞻时瞬时起音是**正确**做法（M14/G.34），漏的是滑窗最小值那一条，已换成单调队列 |
| §B.5 / §C.3「§3.5 与 §4 清单均未开始」 | **多数已完成**：齐奏/立体声展开、键盘跟踪（`PolySynth.keyTrackedCutoff`）、立体声完整性（M3/G.20）、延迟不重建图（M4/G.31）、toolbarTiers（G.10，`probe:toolbar` 在 verify 里）、密度设置（`settings_density_*` + `:root[data-density]`）、首屏单一动作（G.26）、iOS 静音无声（U6）。**仍未做**：波表/鼓组采样层（D.5，需素材与许可）、混响 IR 移出主线程（M5 已降级） |
| 状态块「仍未做：首屏单一动作（U1/U2）…以及术语表、工具栏 `?` 入口、Modal 栈三项（未核实）」 | **四项都已交付**：U1/U2 = G.23 + G.25 + G.26；术语表 = U9（v2.1.3/G.42）；工具栏 `?` = v2.0.89/G.28；Modal 栈 = v2.0.86/G.24。**同一份文档的 U 表早就这么写了，只有那个状态块没跟上** |
| §C.3 第 1–4 条（手机端项） | **随用户冻结**：整节标注为暂停，不再推进；第 3 条的实测尺寸后来更新为 36×40（G.37/G.39） |

### 教训

"计划里写着未做"不等于"没做"——**它只等于"某人某天认为没做"**。这次翻出的六处里五处是过时、
一处是自相矛盾（U 表 vs 状态块）。所以这一轮的产物不是新功能，而是一份**能与代码对上的计划**：
每处更正都附了核实命令或断言来源，下次再读可以直接重跑。

`npm run verify` 全绿（含 3 桌面 + 4 手机目标，以及本轮新接进来的两个音质门禁）。

## G.44 U10 的三条尾巴：参数条键盘、长度作用于选区、琴键可弹（v2.1.5）

### 这一轮不是新功能，是把 U10 自己的验收标准补齐

U10（「卷帘是无障碍空洞」）在 v2.0.93 交付了主体：一个可聚焦表面 + 光标 + 键盘建/删/力度/音长。
那之后计划里一直挂着三条「仍未做」，本轮逐条做掉——每条都先跑失败测试，再改实现。

| 计划里的原话 | 本轮 |
|---|---|
| **力度条本身不可聚焦** | 参数条成为**一个 tab 停留点 + 光标**（roving tabindex）：每列是 `role="slider"` + `aria-orientation="vertical"` + `aria-valuemin/max/now/valuetext`，上下键改值、左右键换列、Shift 与 PageUp/PageDown 走大步、Home/End 到两端，改动走 `role="status"` live region 播报 |
| **力度/音长改由键盘在光标或选区上调整** | 力度**早已**同时作用于光标与选区；长度**只作用于光标**——这是真缺口。本轮补上 `scaleNotesLength`（与 `scaleNotesVelocity` 同形：选区 → 步集合、相对增量、同一处钳位），`[`/`]` 现在对整段选区生效并播报「N 个音」 |
| **卷帘键盘（点击试听）仍是指针** | 左侧钢琴键成为 roving tab stop：上下键走一个半音**并试听**（等价于指针横拖），Shift/PageUp/PageDown 一个八度，Home/End 到两端，Enter/Space 重弹当前键并播报音名；录音待命时与指针路径一样记谱 |

### 顺带发现的一件事：四个维度四套刻度，写了两份

参数条在同一列里画力度（1..127）、音长（0.1..2.0 步）、概率（0..100%）、滚奏（1,2,3,4,8）。
指针路径有自己一套 `commitValue` 换算，键盘要再加一套，柱高与读数是第三套——这正是两条输入路径会
悄悄分叉的地方（拖拽改 100、按键改 101，长期没人会发现）。所以四套刻度收进
`src/features/sequencer/laneValues.ts` **一处**：指针把 0..1 的比例交给 `ratioToValue`，键盘按
`step`/`largeStep` 走值，柱高与读数都取同一份规格。

**换算公式是从组件里搬出来的，不是重写的**：拖拽画到 100 仍然画到 100，测试逐条钉住——
`round(ratio*127)` 的钳位、`round((0.1 + ratio*1.9)*10)/10` 的四舍五入、概率的 `round(ratio*100)`、
滚奏的 0.2/0.4/0.65/0.85 阈值，以及四种柱高与四种读数格式。

滚奏还有个诚实性问题：它的取值是**一张清单**（1、2、3、4、8），不是 1..8 的区间。
所以规格里写的是 `values`，键盘沿清单走（大步跨两项）；导入进来的非法值（比如 5）先吸附到最近的一项
再走，**不会凭空造出一个引擎没有定义的连音数**——这一条有专门的断言。

### 三个「焦点即对象」的细节

三条路径都遵守同一条规矩：**带焦点环的那个东西，才是按键作用的对象**。键盘事件从 DOM 上读列/键的
索引，而不是只信组件 state——两者只有在焦点被别的东西挪走时才会分歧，那时 DOM 是对的。

琴键的 `stopPropagation` 还顺手解决了一件旧事：有选区时，窗口级处理器会移动音符；但焦点在琴键上时，
方向键应当是「走琴键」而不是「移音符」。这一条也写成了测试，免得以后有人把两个处理器合并时忘掉。

### 空转与代价

- 参数条：**0 个新 DOM 节点**（原列加角色与 tabindex），`min-w-[28px]` 的指针手势、`data-playhead`
  激光同步、触摸横扫全部未改；`VelocityLane` 只少了两段本地的换算函数。
- 琴键：**0 个新节点**，只加了 roving tabindex、角色与一个按键处理器。
- 手机端代码一行未动（用户冻结）。手机门禁测的是 `page.goto` 后的默认页面（卷帘未打开），
  所以新增的 `[tabindex="0"]` 不进任何被测量的表面。

### 门禁

| 文件 | 条数 | 钉住的东西 |
|---|---|---|
| `src/test/laneValues.test.ts` | 12 | 指针数值原样、刻度自洽、**比值两端命中 min/max**、离散清单与吸附、按键映射、不吞平台键 |
| `src/test/velocityLaneKeyboard.test.tsx` | 8 | 一个 tab 停留点、slider 语义、上下键写值（含 `0.8+0.1` 的浮点）、钳位、播报、光标移动与聚焦、不吞键 |
| `src/test/pianoRollLane.test.tsx` | +8 | 选区的力度**与音长**、两处钳位（一格 / 一小节）、不吞平台键、琴键 roving、试听、不移动选区 |
| `src/test/rollKeyboard.test.ts` | +3 | 琴键移动与八度、绘制边界、Enter/Space 与不吞键 |

「比值两端命中 min/max」这条不是装饰：音长的 0.1 下限当年就是被漏掉的一角（指针能画出的最矮位置
...
## G.46 工作室两列左右互换：一次我自己制造、并上线了 20 多个版本的回归（v2.1.7）

### 起点是 G.8 第 8 条，而它的前提就不成立

G.8 第 8 条写着「PC/iPad 的走带条会随页面滚走，且 `MobileTransportBar.tsx` 上方的注释明确写着这是刻意的」，
结论是「性质待确认」。**第一步是查证前提**：把「遮挡 / 长页面 / scroll away」在 `src/` 里搜了一遍——
没有任何这样的注释。所以问题不是「有意还是权宜」，而是这条的说法从来没有出处。

### 先量，再判断

写 `scripts/probe_transport_reach.mjs`：在三个桌面/iPad 视口把工作室滚到底，报告走带组的位置、
它中心的命中测试结果、以及哪个祖先会让 `sticky` 失效。结果一致：

| 视口 | 页面可滚 | 走带组滚到底后 top | 中心命中 | 可达 |
|---|---|---|---|---|
| 1440×900 | 594 px | −331 | 无 | ❌ |
| 1194×834（iPad 横） | 676 px | −413 | 无 | ❌ |
| 1280×720 | 774 px | −511 | 无 | ❌ |

**但量测里冒出一个更不对劲的数字**：1440px 宽的视口里，工具栏根节点只有 **318px 宽**。
把工作室网格的子项 dump 出来：

```
1440×900  grid-cols: 352px 1012px
  aside@x400  1012px   ← 信息栏（InfoDossier）占了 1fr
  div@x28      352px   ← 编辑器（SequencerPanel）被挤进 352px 列
```

截图确认：编辑器只剩三格步进、工具栏折成五行的窄条，信息栏占了整个右半屏。

### 根因是我在 v2.0.88 引入的

`git log -S` 指到 `854a01f`（U1「首屏单一动作」）：那一轮为了在面板上方放 `FirstRunPrompt` 与
`SaveIndicator`，把原本**直接作为网格子项**的 `SequencerPanel` 包进了一个
`<div className="min-w-0 flex flex-col">`。`order` 类（`order-1 lg:order-2`）留在了 `section` 上——
而 `section` 现在是 wrapper 的子节点，**不是网格项**。网格项 wrapper 的 `order` 是默认值 0，
`InfoDossier` 的 `lg:order-1` 于是排在它前面，抢到了第一列（352px）。

**这是我自己的回归，而且已经上线 20 多个版本。** 修复是一行：把 `order-1 lg:order-2` 放到 wrapper 上
（`order` 属于网格项，不属于它的子节点）。重建后 1440×900：信息栏 352 左、编辑器 1012 右，
工具栏一行放得下走带与编辑组，步进网格一屏显示 16 格以上。

### 为什么所有门禁都是绿的

因为**所有量测都发生在面板内部**：`probe:grid-gutter` 量的是面板里的冻结列与网格间距（它报出
「frozen column [45, 237) = 192px」——两个数字都在 352px 的窄列里，自洽得看不出问题）；
`probe:toolbar` 量的是面板里的按钮密度与折行；手机面审计量的是手机壳。**没有任何一条门禁比较
「两个网格项谁宽」**，所以一个整体错位的布局能让 190 个文件、2172 条测试、7 个目标全绿。

这是"相对断言在整体错了以后仍然自洽"的教科书例子，也说明量测要有一个**绝对**的锚点。

### 门禁（两条，一条在浏览器里，一条在源码里）

1. **E2E（桌面 3 个 + iPad 2 个目标）**：`main.grid` 是两列时，断言第一个子项是 `aside`、第二个更宽
   且在其右侧、编辑器宽度 ≥ 55% 视口；<1024px 时断言单列且编辑器在上（信息栏在下）。
   **在故意还原回归的构建上实测失败**：
   `The studio's columns are swapped on Desktop Chromium / Chrome: aside@400 852px then div@28 352px (grid: 352px 852px)`
   ——门禁能失败，才叫证据。
2. **`src/test/studioColumns.test.ts`（4 条）**：把 contract 钉在源码结构上——order 类必须在**网格项**上、
   `SequencerPanel` 必须在该 wrapper 内、两列网格与「折叠成单列」的退路都还在。浏览器那条说"被换了"，
   这条说"为什么必须在那层"。

### 仍然开着的一件事（需要拍板）

修完仍是事实：**PC/iPad 上走带条会随页面滚走**（上表数字），手机端是固定的。把它钉在页眉之下需要
显式声明页眉高度（实测 69px）并接受工具栏那一行 **105px ≈ 12% 视口**常驻——这属于产品取舍，
不改代码先问用户。`scripts/probe_transport_reach.mjs` 留在仓库里，改与不改都能立刻重新量。

全量单测 **191 文件 / 2176 用例**，`npm run verify` 全绿（3 桌面 + 4 手机目标 + 音质/布局门禁）。

## G.47 走带条钉在页眉之下；顺带发现 `position: sticky` 在这个应用里从未生效（v2.1.8）

### 用户拍板：只钉紧凑走带条

G.46 末尾把「要不要把走带条钉住」留给了用户，选项里最省的是「只钉紧凑走带条（约 52px）」，
用户选的就是这一条。实现方式是：把工具栏里的**传输组**（播放 / BPM / 小节 / 图案槽 / 撤销重做）
提取成一个独立的 sticky 条，其余组（编辑 / 视图 / 工程）继续随页面滚——常驻高度只有一行，
而不是整条工具栏。

### 第一次实现：条是 sticky，位置一动不动

写完 CSS（`sticky top-[var(--app-header-h)]`）后实测：滚到底时传输组仍在 `top −242`。逐步查下去，
发现根因不在我这条上——**整个应用里没有任何 `sticky` 生效过**：

```
滚轮 400px 之后，1440×900：
  header（声明了 sticky top-0 z-50）  top −400   ← 也没吸顶
  strip                               top −145
```

`index.html` 的 `<body>` 上挂着 Tailwind 的 `overflow-x-hidden`。`overflow: hidden` 会**让元素成为
滚动容器**，于是 body 成了所有 sticky 后代的最近 scrollport；而 body 是随页面一起滚的（它自己内部
并不滚），所以 sticky 元素跟着一起走。运行时注入 `body{overflow-x:clip!important}` 后立刻正确
（header 0、strip 69），用 `!important` 才盖住，是因为工具类 `.overflow-x-hidden` 的特异性高于
样式表里的 `body{...}`——这也是为什么它藏了这么久。

修法：把 `<body>` 上的工具类去掉，改由样式表声明

```css
body { overflow-x: hidden; overflow-x: clip; }   /* hidden 是老 Safari 的回退 */
```

`clip` 裁剪同样的像素，但**不创建滚动容器**，横向拖动页面依然不可能（这条规则存在的理由），
而 sticky 恢复工作。**页眉现在真的会吸顶**，侧栏 dossier 也是。

顺带修掉一处因为「sticky 从来没生效」而没人发现的小错：dossier 的 `top-16`（64px）比页眉实际高度
（69px）少 5px，现在两个 sticky 元素都用 `top-[var(--app-header-h)]`。

### 两条门禁，都做了失败可证

**E2E（桌面 3 + iPad 2 个目标）**，三件事一起断言：

1. `--app-header-h` 必须等于页眉的实测高度（页眉高度由内容决定，会漂）；
2. 滚到页面底部后，传输组必须在视口内，**且它中心的 `elementFromPoint` 必须命中它自己**——
   「在视口里但被页眉盖住」是点不到的，纯几何断言会放它过去；
3. 它必须停靠在页眉之下 0–8px（太大是空隙，太小是被盖住）。

**失败可证**：把 `sticky` 从条上拿掉重建，E2E 报
`The transport is not reachable with the page scrolled on Desktop Chromium / Chrome: top −462, at its centre nothing`。

**`src/test/stickyAnchors.test.ts`（7 条）**把「让 sticky 可能」的四件事钉在源码上：`<body>` 不得
再出现 `overflow-x-hidden`；样式表必须用 `overflow-x: clip`（且保留 `hidden` 回退在前）；两个 sticky
元素必须锚在 `--app-header-h` 上；走带条必须是**工具栏的同级兄弟**而不是子节点——sticky 元素只能在
自己父盒内移动，而工具栏只有 116px 高，之前放在里面只有 63px 的可移动距离，这就是第一版失败的原因。

### 顺手暴露的一处探针腐化

`probe:toolbar` 一直用「传输组的最近 `div.flex.flex-col`」来定位工具栏，并注释说"工具栏被搬过家，
所以不靠 wrapper testid"。这次正是这个假设坏了：条搬出去之后，那次向上查找越过工具栏爬进了面板
外层，报出 **155 个控件 / 1105px 的"工具栏"**（122.7% 视口）。现在工具栏与条都有稳定 testid
（`studio-toolbar` / `toolbar-transport-strip`），探针量两者的并集：**16 控件 / 2 行 / 110px**，
与改前一致——也就是说常驻的只有条这一行，其余布局没有变胖。

### 门禁与代价

- 常驻高度：条 53px（≈6% 视口），工具栏总高 110px 与改前 111px 相差 1px（条本来就占那一行）。
- 手机端尚未解冻，代码一行未动；手机用不到工具栏，条只出现在 PC/iPad。
- 全量单测 **192 文件 / 2185 用例**，`npm run verify` 全绿（3 桌面 + 4 手机目标 + 音质/布局/预算门禁）。

## G.48 834px 的页眉有八个控件落在屏幕外：`overflow-x: hidden` 的「程序化可滚」掩盖了它（v2.1.8）

### 怎么发现的

G.47 把 `<body>` 的 `overflow-x: hidden` 换成 `clip`（sticky 的前提）之后，E2E 的 **iPad 竖屏**
目标立刻以 `elementHandle.click: Element is outside of the viewport` 失败——失败点是"切换语言"。

### 量测

页眉的内容宽度是**固定的 1153px**，与视口无关：

| 视口 | 页眉内容 | 落在视口外的控件 |
|---|---|---|
| 834（iPad 竖） | 1153 > 834 | **8** 个：搜索、随机、语言、设置、版本、帮助、上手引导、快捷键 |
| 1024 | 1153 > 1024 | 4 个：版本、帮助、上手引导、快捷键 |
| 1194（iPad 横） | 1194 | 0 |

**iPad 竖屏的用户此前根本够不到语言切换与设置面板**——而 E2E 一直是绿的，因为
`overflow-x: hidden` 仍然允许**程序化**横向滚动：Playwright 的 `elementHandle.click()` 把 body
横向滚过去、点中了按钮，而没有任何手指能到那里。换成 `clip`（完全不可滚）后，同一句 click 失败，
这才是它被发现的原因。**一个把缺陷藏起来的「能滚」，比缺陷本身更值得记录。**

### 修法：换行，而不是裁掉

页眉根节点加 `flex-wrap`（并把 `gap-4` 拆成 `gap-x-4 gap-y-2`），内容装不下就换行。实测：

| 视口 | 768 | 834–1100 | ≥1194 |
|---|---|---|---|
| 页眉高 | 145px | 107px | 69px |
| 视口外控件 | 0 | 0 | 0 |

高度因此不再是常数，所以 `--app-header-h` 改由页眉在挂载时（以及每次 ResizeObserver 触发时）
把**实测**高度写进根元素；样式表里的 69px 只是首帧值。旋转、拖窗口都会跟着更新。
这比在样式表里写三档媒体查询更诚实：档位是布局的**结果**，不是它的输入。

### 门禁

1. **E2E（桌面 3 + iPad 2）**：遍历页眉里的按钮/链接/输入，任何一个落在视口外即失败并报出是哪几个；
   同时断言页眉 `scrollWidth <= clientWidth`——不靠祖先裁剪来"看起来没有溢出"。
2. **走带可达性检查改成「面板顶部 + 200px」再断言**：sticky 元素被父盒夹住，在**页面最底部**
   （竖屏平板上只剩面板最后 60px）它本就该随面板离开；正确的断言是"编辑器在屏幕上时，走带条在屏幕上"。
   同一处补了一句 `Escape`：之前它在 Chromium 上命中了一个**没关掉的页眉下拉菜单**，所以顺手把
   命中对象的 class 也写进错误信息（现在会打印 `div.flex.items-center.gap-1.5` 这种证据）。
3. **`stickyAnchors.test.ts`（9 条）**：新增「页眉必须 `flex-wrap`」与「必须用 `ResizeObserver` 写
   `--app-header-h`」两条，把这次的修法与理由一起钉住。
...
## G.51 钟的不谐分音：从「两个正弦」到真正的金属（v2.1.11）

### 计划里的说法，核实后比它写的更糟

计划记着「不谐分音：`createPeriodicWave` 只能生成**谐波**级数，所以钟/锣/钟琴的金属感只能用两个
不成整数比的振荡器近似（`bell_lead` 现在就是这么做的）」。核实发现连那个近似也不成立：`bellLead`
的第二振荡器是 **+1900 cents**，而 1900 cents = 2.997:1——**就是第三谐波**（只是缺了第二谐波）。
那是一张标准谐波谱，听起来像管风琴，不是钟。**「不成整数比」这句话本身是错的。**

### 做法

预设表新增 `partials`（非整数比的偏音：ratio / gain / decayScale）与 `partialsLevel`；voice builder
对这种预设建**每个偏音一个正弦**的 bank，**两个一组**汇入（树形）。取管钟的一组分音：

| 分音 | hum | prime | tierce | quint | nominal | upper |
|---|---|---|---|---|---|---|
| ratio | 0.56 | 1.0 | 1.19 | 1.5 | 2.0 | 2.74 |
| decayScale | 1.0 | 1.0 | 0.8 | 0.65 | 0.5 | 0.35 |

四个非整数比（0.56 / 1.19 / 1.5 / 2.74）就是「金属」，而**越高的分音衰减越早**是「钟在响」而不是
「嗡嗡响」的原因。

**为什么是两个一组**：一个节点把 3 个以上**不同频率**的振荡器相加时，Chrome 的
`OfflineAudioContext` 渲染不可复现——这是实测性质（10 次渲染的哈希：1/2 个振荡器 = 1 种；
3/4/5/6 个不同频率 = 3/7/9/10 种；3 个 buffer source = 1 种；4 个振荡器分两个节点 = 1 种），
记在 `scripts/diagnose_repeat_determinism.mjs --primitives`，由 `oscillatorFanIn.test.ts` 守着。

### 电平按「能量」配平，不按幅度和

六个分音分摊同一个峰值增益、且高端早衰，同幅度下会低好几分贝。所以 bank 按
**Σ gain² · decayScale** 对旧的「0.8² + 0.4²」配平：`1.2 · √(0.80 / 0.242) ≈ 2.18`。这是**混音平衡**的
理由（同一句话在代码注释里）。

### 门禁

1. **`oscillatorFanIn.test.ts` 从 39 条扩到 91 条**：把**整张合成器预设表**都建一遍音色送进规则。
   这个文件自己的注释早就写着「这条规则是为了阻止下一次『再加一个偏音让音色更丰富』的改动」——
   而它此前只覆盖鼓。另加一条**自检**：手工搭一个「3 个不同频率振荡器汇入一个 gain」的图，必须被
   报出来；没有这条，扩展过的规则仍然可能是空转。
2. **`inharmonicPartials.test.ts`（13 条）**：偏音表的归一化、上限（6）、衰减默认值与非法值；图上
   「每个偏音一个正弦、频率 = f0 × ratio、不得走 PeriodicWave」；**不谐性谓词**（ratio 距最近整数
   > 0.1 才算金属，铃上必须命中 0.56/1.19/1.5/2.74 四个）**以及谓词自检**（喂 `[1,3]` 与 `[1..5]`
   必须判为「没有金属」——旧钟的样子必须通不过）；同音高两次调度逐项一致、不同音高不同；没有
   `partials` 的预设仍然只建 2 个振荡器。
3. **音色基线必须重录**（实测：bank 电平 2.18 → 0.001 会改变 `kawaii-future-bass` 的 `bandDb` 与
   质心，说明这个音色在**音色测量路径**里）。已用 `measure_genre_timbre.mjs` 全量重录 159 曲风。

### 顺带发现：响度基线对 5 个曲风已经过时（与本次改动无关，下一轮修）

> **更正（v2.1.12 / G.52）：下面这张表的数字来自子集测量，是「首帧走限幅器回退路径」造成的假象，
> 幅度不可信。全库同口径的真实漂移是 mean |Δ| 0.256 dB、max 2.71 dB、136/159 在 0.35 dB 内；
> 而且今天的离线渲染有 9 个曲风突破 −1 dBTP 天花板（限幅器未生效），修好前不重录。见 G.52。**

做基线影响评估时量出来的：

| 曲风 | 基线 arrangedLufs | 现在 | Δ |
|---|---|---|---|
| chicago-house（对照） | −11.694 | −11.726 | −0.03 |
| deep-house（对照） | −13.151 | −13.111 | +0.04 |
| hard-techno（对照） | −11.733 | −11.737 | 0.00 |
| kawaii-future-bass | −11.770 | −15.546 | **−3.78** |
| idm | −11.407 | −12.393 | **−0.99** |
| trap-rap | −12.430 | −12.996 | **−0.57** |
| chicago-drill | −12.331 | −15.175 | **−2.84** |
| brooklyn-drill | −12.017 | −12.768 | **−0.75** |

**A/B 证明与钟无关**：旧预设、新 bank 2.18、新 bank 0.001 三次渲染 `kawaii-future-bass`，arrangedLufs
都是 **−15.546**（而且响度测量路径根本**不渲染**这个音色——它用的是"arranged"渲染，钟不在其中）。
对照组三个曲风与基线差 ±0.04 dB，说明测量本身没坏：**是这 5 个曲风的基线不再描述现实**。

后果与修法：它们的 commit trim 是按旧测量拟合的（−5.01 / −6.91 / −0.07 / −2.66 / −4.30 dB），而
trim **只能压不能提**，所以它们现在比自己的类别目标低最多 ~3.8 dB。修法是全量重录响度基线
（159 曲风）+ 更新 `genreMix.ts`——**单独一轮**，不把「铃的音色」和「159 条 trim」混进同一个版本。

全量单测 **196 文件 / 2270 用例**，`npm run verify` 全绿。

## G.52 量测装置的两处修正；并拦下一个更严重的发现：离线渲染的限幅器对热素材不生效（v2.1.12）

### 这一轮本来要做的事

G.51 末尾写着：响度基线对 5 个曲风过时（−0.57…−3.78 dB），修法是全量重录 + 更新 trim。

### 第一次全量重录，拦下了一个真问题

重录跑完后 `check:loudness` 报出 **9 个曲风超过 0 dBFS 采样峰 / −1 dBTP 天花板**，最差
**tropical-house +0.95 dBTP（这是在 trim 之后）**，而它的 `arrangedTruePeakDb` = **+1.75 dBTP**——
也就是说**限幅器在离线渲染里没有生效**。同一个曲风在已提交的基线里是 −13.01 LUFS / −1.3 dBTP；
今天重录是 −10.30 LUFS（**响 2.7 dB**）/ +1.75 dBTP。**一个不受限的渲染既更响又更亮**，所以"基线过时"
的读数里混着这个缺陷，不能据此重录。

- **复现**：`node scripts/measure_genre_loudness.mjs --genres=tropical-house --out=scratch/x.json --port=3170`
  → 每次都是 arranged +1.75 dBTP、trim 后 +0.95 dBTP。
- **A/B 排除我自己的 M14**：把 `src/audio/MasterLimiter.ts` 与 `public/limiterWorklet.js` 换成
  `b98d5d2^`（单调队列改写之前）的版本，结果**逐位相同**（trim 后 +0.95 dBTP）→ **不是队列改写的锅**，
  是限幅器是否真正接进离线渲染路径、或它在热素材上的行为（lookahead/释放/契约）出了别的问题。
- 因此**这份重录没有发布**（否则就是把削波写进基线）：`scripts/loudness.baseline.json` 已还原为
  已提交版本，`check:loudness` 重新全绿。顺序必须是：**先修限幅器 → 再重录 → 再更新 trim**。

### 同一轮真正的产物：量测装置的两处修正

**1. 首帧回退（冷启动假象）。** 全新页面里的**第一次**离线渲染不走限幅器 worklet（模块惰性注册），
落到 DynamicsCompressor 回退路径；对顶到天花板的素材差 **2.87 dB**：`kawaii-future-bass` 冷渲染
**−15.55 LUFS**、热渲染 **−12.68 LUFS**（各测三次，稳定复现）。测量脚本现在先做一次**丢弃的预热
渲染**再开始，两个分支（全量与 `--sample`）都覆盖。导出路径本来就有对应处理（`limiterKind:
"fallback"` 会明确告诉用户"这份文件比试听更响、控制更差"）。

**这条假象正是 G.51 那张表的来源**：子集运行（`--genres=…`）的"唯一一次渲染"就是冷的那次，所以
单曲风/小样本的**绝对值**永远不可以与全库基线比较。

**2. `check:loudness:fresh`（新，`--sample=N`）。** 现有响度门禁只把报告与**它自己**、以及与
`genreMix.ts` 比对——**一份过时的报告在这些检查下完全自洽**，本次就是这样藏了很久。新模式按**全库**
口径（完整目录 → 同一个 master makeup）重渲染 N 个**均匀分布**的曲风并与报告比对，超过 ±0.35 dB 即
失败。纯逻辑在 `src/utils/loudnessFreshness.ts`（抽样必须覆盖全库、确定性、漂移按幅度排序、缺行也算
漂移），`loudnessFreshness.test.ts` 9 条单测，阈值取"两帧噪声 ~0.05 dB"与"本次真实漂移最小值 0.4 dB"
之间。抽样覆盖是刻意的：本次漂移是**家族状**的（同一批热素材），取前缀会全部漏掉。

**但它暂时不接进 `verify`**：在限幅器修好、基线重录之前它必然红（已用真实数据证明它会失败：
对着旧基线报 `kawaii-future-bass: report −11.77, now −15.55, Δ −3.78`）。脚本与 npm script 都留着，
下一轮修完立刻接线。

### G.51 的更正（重要）

G.51 那张"5 个曲风 −0.57…−3.78 dB"的表**数字不对**——它是上面的冷启动假象。全库同口径的真实比较
（已提交基线 vs 今天的全量重录，159 曲风）：**mean |Δ| 0.256 dB、max 2.71 dB、136/159 在 0.35 dB 内**。
最响的几个漂移是 tropical-house +2.71、cool-jazz +2.55、cumbia +2.31、motown +2.21、bebop +2.07；
G.51 点名的 5 个曲风**确实也在漂**（chicago-drill −2.11、kawaii −0.90、trap-rap −0.58、idm −0.37、
brooklyn-drill −0.24），但幅度与"最差 −3.78"都不可信。**而且这个读数里混着限幅器缺陷**，所以在它修好
之前不据此重录任何东西。G.51 里仍然成立的是：钟与这些数字无关（A/B 的三次渲染都是 −15.546——那也正是
冷启动值）。

**教训（两条）**：① 用不同装置（不同目录规模）量出来的绝对电平不可比较，比较必须在**同一口径**下做；
② 门禁如果只做"报告与自己一致"的检查，它会把一份过时的报告一路放行到底。

## G.53 离线限幅器补上天花板；修好量测装置后重录响度基线；并发现「长页面渲染漂移」（v2.1.13）

### 1. 先修 G.52 拦下的缺陷：离线渲染的天花板必须是渲染器的性质

G.52 只做到「拦下来、不发布」，没有修。这一轮补上：`renderPatternOffline` 在
`await graph.limiter.ready` 不是 `worklet` 时，把渲染好的通道再过一遍**限幅器 worklet 自己的内核**
（`applyOfflineCeiling()` → `limitBuffers(..., { ceilingDb: MASTER_LIMITER_INTERNAL_CEILING_DB })`，
即 −1.3 dBTP，保留 0.3 dB 检测裕量），worklet 路径原样返回。

**A/B 是可复现的，不是「有时」**：G.52 的复现是掷硬币（同一命令两次相差 4.8 dB），所以这一轮先给量测装置
加了 `--force-fallback`（见下），把「平台装不上 worklet」变成确定事件，再对比：

| 六个热曲风（`--force-fallback`） | 无守卫 | 有守卫 |
|---|---|---|
| arranged 真峰最差 | **+2.22 dBTP**（cumbia） | −1.30 dBTP |
| trim 后真峰最差 | **+0.95 dBTP**（tropical-house） | −1.30 dBTP |
| 超过 −1 dBTP 的行 | **6/6** | 0/6 |

无守卫那一列里的 **tropical-house +0.95 dBTP** 正是 G.52 里那份被废弃重录发布的数字——也就是说那个数字
不是噪声，是这条路径的稳定输出。单测 10 条（热素材压到 −1.3 与 −1.0 以下、报告减益、安静素材不动、
幂等、不改输入、长度/延迟与 worklet 内核一致、不把安静声道推高、两条源码级接线），把守卫拆成恒等函数后
**10 条里 5 条失败**。

顺带被既有套件抓到一个测试夹具的洞：`FakeAudioBuffer` 没有 `copyToChannel`，于是所有走回退路径的导出用例
在断言之前就 `TypeError`（4 条）。补上真实语义（含 `startInChannel`）。

### 2. 量测装置的三处修正

- **每行都要说自己走的是哪条限幅器路径**。G.52 的教训是「报告不写路径就没法解释」，但那一轮只做到进程级
  探针；现在每行写 `limiterKind`（同一曲风的 arranged 与各 trim 轮次的**并集**：`worklet` / `fallback` /
  `worklet+fallback`）。
- **`--force-fallback`**：把 `AudioWorklet.prototype.addModule` 换成必定 reject。先试过 `page.route`
  把 `/limiterWorklet.js` 打到 404——**没用**，模块是音频渲染线程取的，Playwright 的路由看不到它，装了路由
  的报告仍然行行 `worklet`。写入注释里，免得下次再试一遍。
- **`assertSingleLimiterPath`**：full run / `--sample` 都要求所有行是纯 `worklet`，否则**逐曲风 fail-fast**
  中止且不写报告。理由不是洁癖：回退路径是另一个渲染器（G.14 实测整体响 2.36 dB），混在一起的行不是同一个
  测量。诊断用的 `--allow-mixed-limiter` 不用于发布。自检：把 bypass 条件改窄后，`--force-fallback
  --sample=3` 报 `3 of 3 genre(s) did not render through the worklet limiter`，并且顺带暴露了一个误判——
  强制回退时采样检查会把「路径不同」读成「报告过时」（+0.67 dB）。

### 3. 修完装置、重录之前：第一次重录的行**不可复现**，于是找到了更根本的东西

3 个均匀采样的曲风里 2 个和刚录的基线差 0.7–2.5 dB，而 `chicago-house` 只差 0.16 dB。用两个诊断脚本查下去：

- **同一页面内是确定的**：`alternative-rock` 在新页面连测 3 次都是 −12.711 LUFS；同一页面渲染过 118 个别的
  曲风之后再连测 3 次都是 **−10.259**（正是那份重录发布的值），reload 之后又回到 −12.711。
- **起跳点在 45–75 次渲染之间**（`scripts/probe_page_degradation.mjs`，每 15 次测一次哨兵）：

| 本页已渲染次数 | 0 | 15 | 30 | 45 | 60 | 75 | 90–135 |
|---|---|---|---|---|---|---|---|
| `alternative-rock` LUFS | −12.711 | −12.710 | −12.831 | −12.711 | **−10.805** | **−10.259** | **−10.259** |

- **特征**：真峰仍钉在 −1.30 dBTP（限幅器在跑）；RMS 只差 1.16 dB 而 LUFS 差 2.45 dB（差异集中在高频）；
  `legacy`（完全不走混音表的那一遍）同步偏 2.59 dB，说明是整条渲染链的输出变了，不是混音差异；图案对象没有
  被改写；`limiter.dispose()` + `ctx.close()` **挡不住**（所以不是限幅器节点没释放）；reload 能完全清除。
- **对装置的意义**：一个两小时的 run 会在中途悄悄换掉「它测量的是什么」。所以第二次重录用
  `--reload-every=12`（对照实测起跳点 45–60，留 ~4 倍余量）每 12 次测量换一次页面、换完丢弃一次预热渲染，
  并每次都重测**哨兵** `alternative-rock`（漂移 >0.3 dB 即中止、不写任何东西）。哨兵容差的自检：把容差设成 0
  时运行会按预期中止并打印对比值。
- **仍然未解**：根因没找到（浏览器进程级还是应用级缓存未定）。它**对产品也有影响**——同一会话里连续导出足够
  多次（每次导出至少一次离线渲染）就可能越过同一阈值，导出的文件会和试听不同。这条按「已知未解」留在
  G.53 末尾，不假装已经解决。

### 4. 新基线（159 曲风）

跑完整一轮（159 曲风 × 5 轮 trim 拟合，约 2.5 小时；本轮共约 **711 次测量**，按每 12 次回收一次 → 约 59 次页面回收，日志里出现 **52 次「冷渲染落到回退路径」告警**，说明回收后的丢弃预热确实在跑）：

- **159/159 行测到，全部 `worklet`**，`assertSingleLimiterPath` 一次都没触发；哨兵没有中止过（代码里 >0.3 dB 就 `exit(1)` 且不写报告），但这次运行的日志**没有逐次记录哨兵值**——逐次记录是这一轮之后才加的，所以「稳定」的正面证据是下面 §5 的抽样复现（12/12 Δ 0.00 dB），不是哨兵日志。
- **0 个 clamp**；**0/159 超过 −1 dBTP**，真峰最差 **−1.275 dBTP**（funk）。
- `after` 的库内 p90−p10 4.02 dB（门禁按**类别目标**判定，库内离散只作信息展示：6 个类别目标中位数分别落在 Electronic −12.91 / Rock·Metal −10.79 / Hip Hop −14.87 / Jazz·Blues −14.24 / Pop·R&B −13.26 / Latin·World −14.02，对应目标 −13 / −10.5 / −15 / −14.2 / −13.2 / −14）。
- 86/159 个曲风被自身 crest 顶住（物理限制，不是拟合错误）。

**旧基线是「降级页」量出来的**（这是这一轮最重要的结论，也解释了 G.51 那批「基线过时」的读数）：

| 曲风 | 已提交 arranged | 长页面（污染）| 新基线（新页面，可复现）|
|---|---|---|---|
| ambient | −13.77 | −13.63 | **−18.11** |
| kawaii-future-bass | −11.77 | −12.68 | **−15.55** |
| ambient-dub | −14.40 | −14.52 | **−17.77** |
| melodic-dubstep | −13.42 | −13.39 | **−16.72** |
| chillwave | −17.04 | −17.31 | **−19.65** |
| alternative-rock | −10.42 | −10.26 | **−12.71** |

差异方向解释了 G.51 的症状：污染值**偏响** → 拟合出的 trim **偏狠** → 实际播出比类别目标低最多 3.8 dB。最极端的例子是 `ghetto-house`：它的 `target` 就是它自己能达到的 arranged 值（`targetCappedByDynamics`），按理 trim 应该是 0，但已提交的 trim 是 **−7.84**——因为它是在降级页里「测」出来的。新基线里 77 条 trim 非零（旧 96 条），**71 个曲风变响、19 个变安静**，最大 +7.84 dB（ghetto-house）、−6.46 dB（microhouse）。

### 5. 接线与验证

`check:loudness:fresh` 接进 `npm run verify`（`--sample=3`，±0.35 dB，约 60 秒）。对着新基线：

- 3 个抽样曲风 Δ **0.00 / 0.00 / 0.00 dB**；
- 扩到 `--sample=12`（全库均匀抽样）同样 **12/12 Δ 0.00 dB**。

也就是说：**新基线描述的就是「新页面现场渲染」的结果**，门禁量的和基线量的是同一个东西——这正是旧基线缺的那一条。

### 5. 接线与验证

- `npm run check:loudness` 全绿（159 行 trim 与报告一致、0 clamp、真峰最差 −1.275 dBTP、类别目标无一超出 1 dB）。
- `npm run check:loudness:fresh` 全绿（3/3 Δ 0.00 dB；`--sample=12` 时 12/12 Δ 0.00 dB）。
- `npm run verify` 全绿（详见 release commit）。
...
## G.55 追到 G.53「长页面渲染漂移」的根因：GS-1 的 WASM 内存预算（v2.1.14）

G.53 只做到「量到了、并且在量测装置里绕开」，根因留白。这一轮把它追到底了。

### 定位过程（每一步都是可复现的测量，不是推断）

1. **按轨道拆开**（`scripts/probe_render_drift_stems.mjs`，同一页面里先量一遍、渲染 60 个别的曲风、再量一遍）：

   | 轨道 | 前 | 后 | Δ |
   |---|---|---|---|
   | bass / fx | −14.646 / −66.851 | −14.646 / −66.851 | **0.000 dB** |
   | chords | −27.374 | −14.324 | **+13.050 dB** |
   | lead | −30.188 | −46.743 | **−16.555 dB** |
   | master | −18.113 | −13.628 | +4.485 dB |

   即：**只有 GS-1 负责的两个角色（chords / lead）变了**，native 合成器的轨道逐位相同；limiter 两次都是
   `worklet`，真峰都钉在 −1.30 dBTP。配置维度也一致：`makeup −12`（限幅器完全不介入）同样漂 +1.849 dB，
   所以差异在**内容**，不在母带链。

2. **让渲染器自己说**：把 `renderPatternOffline` 里 host 创建的 `catch` 临时改成打印原因（一行诊断补丁）：

   ```
   [gs1-probe] track 5 attempt 1 failed after 91 ms:
     Error: [Gs1Host] worklet error: WASM 实例化失败:
     RangeError: WebAssembly.instantiate(): Out of memory: Cannot allocate Wasm memory for new instance
   ```

   同时 `onGs1HostFailures` 从 **0 → 2**。所以「漂移」的真身是：**页面用尽了 WASM 内存，GS-1 装不上，
   渲染器把 chords/lead 换成了内置合成器**。

3. **量化预算并测试能否释放**（`scripts/probe_gs1_memory_release.mjs`；每个策略开一个全新浏览器进程——第一版
   复用同一页面，导致第一个策略之后全部「0 built」，什么都测不出来）：

   | 拆解策略 | 建成的 host 数 | 首个失败 |
   |---|---|---|
   | keep（只丢引用） | **124**/300 | #124 |
   | keep + 显式 `gc()` | **125**/300 | #125 |
   | `host.dispose()` | **125**/300 | #125 |
   | dispose + `gc()` | **124**/300 | #124 |
   | dispose + `ctx.close()` | **124**/300 | #124（`close` 在 `OfflineAudioContext` 上**根本不存在**） |

   结论：**一个页面能实例化约 124 个 GS-1 host，之后无论怎么拆都拿不回来**（WASM 内存由渲染进程持有，
   只有页面卸载才释放——这解释了两件事：为什么 reload 能恢复，以及为什么 `limiter.dispose()` 实验当时无效）。

### 修法与防线

1. **渲染器：stem 渲染不再为用不到的轨道建 host。** 调度的第一行就会丢掉非本轨的轨，但 host 循环过去对
   *每个* GS-1 角色都建一个（`GS1_ROLES = ["chords","lead"]`），于是每渲染一条 stem 白建 2 个 host。一次
   「母带 + 8 stems」的导出因此花 18 个 host，其中 14 个永远不可能出声；修完后是 **4 个**，一个页面能做的
   导出次数从约 7 次变成约 31 次。`src/test/gs1StemHosts.test.ts`（4 条）钉住；把那一行 `continue` 删掉，
   3 条立刻红。
2. **量测装置：任何一行丢了 GS-1 音色就整轮中止、不写报告**（`assertNoVoiceFallback`，与
   `assertSingleLimiterPath` 同构，逐曲风 fail-fast，诊断用 `--allow-voice-fallback`）。这正是本轮重录被污染
   的那一类：那一行看起来完全正常（峰值在 −1.3 dBTP、没有 clamp、trim 与报告自洽），只有这个计数说明
   chords/lead 换了个合成器。失败可证：临时把失败数 +1，`--sample=1` 立刻报
   `1 of 1 genre(s) lost a GS-1 voice`。
3. **报告里每行写 `gs1HostFailures`**，与 `limiterKind` 同理——「报告不写它，就没法解释它」。
4. **用户侧本来就已经是诚实的**：`export_wav_degraded_gs1` 会告诉用户「有 N 个 GS-1 音色未能加载……
   请刷新页面后重新导出」（上一轮加的）。这一轮补上的是**别让这件事悄悄污染数据**，以及**让它更晚发生**。

### 仍然存在、且无法在应用侧根治的部分

页面级 WASM 预算用尽后，GS-1 就只能退化到内置合成器，直到用户刷新页面。应用侧能做的只有：少建 host（本轮做了
8 倍削减）、及时而明确地告诉用户（已有）、以及不要在数据基线上假装没发生（本轮补了门禁）。真正的根治要么在
GS-1 上游（降低每个实例的 WASM 预留），要么在宿主（一个可回收的离线渲染 worker），两条都超出本轮范围，
按「已知限制」记录：**同一会话里导出约 30 次以后，chords/lead 会退化为内置合成器，刷新页面恢复。**

# M · 手机端独立 UI（v2.2.0 起，取代此前的「手机端冻结」）

用户决定：手机端不再冻结，按手机特点**重新设计一套独立 UI**，不再沿用桌面 IA。术语沿用既定约定
（工作台 / 律动星系 / 音长）。
...
| # | 模块 | 内容 | 来源 |
|---|---|---|---|
| 1 | **首页** | 曲风库（列表/筛选/搜索）＋ **播放条**；点卡片进曲风详情 | `mobile_all.html` 的 `#page-styles`（曲风库）+ 播放条为新增 |
| 2 | **即兴** | 律动编排网格 + 打击垫 + 录制/播放；**BPM 条移到底部**；去掉底/军/镲等鼓组静音行；**不显示播放条** | `mobile_all.html` 的 `#page-jam` 精简 |
| 3 | **挑战** | 盲听辨曲（难度、天梯积分、四选一、解析） | `mobile_all.html` 的 `#page-quiz` + 现有 `ChallengeView` 内容 |
| 4 | **探索** | 底鼓设计 / 和弦走向 / 律动解构 三个子页；重做手机与横竖屏适配；去掉已不存在的入口（如「载入工作台」） | `mobile_all.html` 的 `#page-lab`（底鼓）+ 现有 Kick/Chords 视图 |
| 5 | **更多** | 设置、更新日志、帮助、语言、关于等杂项 | 现有 `SettingsModal` / `UpdatesModal` / 帮助内容 |

底部 tab 顺序：**首页 · 即兴 · 挑战 · 探索 · 更多**。五个是 390 px 宽度下「11 px 标签 + 44 px 触控目标」的
上限（与现有 `MobileTabBar` 的结论一致）。

## M.2 视觉与交互语言（取自两份参考稿）

- 深棕/琥珀：`--bg #0B0907`、`--card #14110B`、`--card2 #1B160D`、`--line rgba(238,225,200,.09)`、
  `--line2 rgba(238,225,200,.2)`、`--ink #EFE7D6`、`--ink2 #A79B82`、`--ink3 #6F6653`、
  `--gold #E9A23B`、`--gold-hi #FFCF7D`、`--teal #3FD8C2`、`--red #F26D6D`、`--green #5AD48E`；金色底上的文字
  一律 `#181104`。
- 字体：标题/正文 Space Grotesk（已加载），所有数字与元信息用等宽字体（现有 JetBrains Mono；参考稿用的是
  IBM Plex Mono，本轮**不新增字体依赖**，以 JetBrains Mono 代替并记录差异）。
- 层次：16 px 页面边距，卡片 `border-radius:16px`、`margin-top:10px`，`zhead`（14 px 标题 + 等宽小字英文标签），
  chips/胶囊用 `border-radius:99px`，圆按钮 44–54 px，滑块轨道 2 px + 20–24 px 圆形滑块。
- 背景：`body::before` 顶部金色径向光晕 + `body::after` 5% 噪点（`feTurbulence` data-URI，`z-index:99`）。
- 动效：页面进入 `rise .45s cubic-bezier(.22,1,.36,1)`；抽屉 `.38s cubic-bezier(.22,1,.36,1)`；
  `--bpmBeat = 60/bpm` 驱动按钮/圆点/EQ 的节拍脉冲（与播放同步）。
- 手机约束（已有基础）：`viewport` 已 `user-scalable=no` + `maximum-scale=1.0`（禁双指缩放与 iOS 放大镜）；
  所有可点元素用 `pointerdown`/`click`、`touch-action:manipulation`，输入类控件字号 ≥16 px 以免 iOS 聚焦缩放；
  `env(safe-area-inset-*)` 处理刘海与home指示条；`-webkit-tap-highlight-color:transparent`、`user-select:none`。
- 横竖屏：参考稿**完全没有**响应式/方向处理（固定 432 px 列）。本项目要求横竖屏都友好，因此竖屏按单列，
  横屏按「左内容 + 右控制」或两列网格重排——这是**净新增的设计**，不是移植。

## M.3 播放器（全屏 + 播放条）

- **播放条**（常驻底部，位于 tab bar 之上）：左侧封面/曲风色块 + 曲风名/BPM + 播放/暂停；点击任意处进入全屏。
- **全屏播放器**：按 `player2.html` 的黑胶 + 唱臂 + 旋转动画与交互实现（canvas 绘制、DPR 上限 2、唱臂弹簧、
  搓盘改 BPM、`--kick/--breath` 驱动全局 CSS 装饰、`--bpmBeat` 节拍脉冲、`rise` 入场）。
- **抽屉式隐藏**：全屏时播放条上的元信息（进度、BPM ±、模式、列表入口）收进一个上滑抽屉
  （参考 `mobile_all.html` 的 `.sheet`：`translate(±105%) → 0`，`.38s cubic-bezier(.22,1,.36,1)`，抓手条 36×4）。
- **单击唱片 → 曲风详情**：唱片区域点击切换到该曲风的详情页；再点返回唱片视角（对应网易云的「查看歌词」逻辑）。
- **左上角向下箭头**：点击收起为播放条；再点播放条回到全屏。
- **左侧模式按钮**：在「单曲循环 → 大曲风内循环 → 全部曲风随机」三者间循环，图标与文案随模式变化并持久化。
- 播放器状态与音频时钟**单一真相源**：唱片角度由引擎的（步进 + 步内比例）推导，绝不用 CSS 动画自转，
  否则与声音漂移。

## M.4 曲风详情

每个曲风都有详情页；所有出现曲风名/封面的地方都能一处点到详情（列表卡片、播放条长按/点角标、播放器唱片、
即兴页的伴奏名、挑战页解析里的曲风名）。详情内容：名称（中英）、年代/分类、BPM 区间、拍号/摇摆、
听感六维、DNA 一句话、轨道构成、代表曲目/试听、相近曲风（可跳转）、「去即兴」「加入对比」等动作。

## M.5 交付切片（每片都带可失败测试 + 保持 verify 全绿）

1. **M1 外壳**：`src/mobile/` 设计令牌 + 五格底部栏 + 路由（URL 可分享、支持浏览器返回）+ 页面切换动效；
   接入 `App.tsx` 的 `isMobile` 分支（桌面不受影响）。
2. **M2 首页**：曲风列表（分类 chips、搜索、卡片：色块 + 中英名 + 年代/BPM/律动元信息）+ 播放条 +
   曲风详情页。
3. **M3 播放器**：全屏黑胶（canvas 渲染器拆成纯函数以便单测：几何/角度/唱臂/搓盘）+ 抽屉 + 模式循环 +
   收起/展开 + 点唱片切详情。
4. **M4 即兴**：编排网格 + 打击垫 + 录制/播放 + 底部 BPM 条。
5. **M5 挑战**：难度/天梯/四选一/解析（复用现有 `ChallengeView` 的题库与判分逻辑）。
6. **M6 探索**：底鼓设计 / 和弦走向 / 律动解构 三子页 + 横竖屏适配。
7. **M7 更多**：设置、更新、帮助、语言、关于。
8. **M8 收口**：E2E 手机断言改为新 IA（触控目标 ≥44 px、横竖屏、无缩放/放大镜、五格栏可见），
   文档与变更日志，发布 v2.2.0。
